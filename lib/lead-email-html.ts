  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:${C.bg};padding:28px 16px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:${C.card};border:1px solid ${C.hairline};border-radius:14px;overflow:hidden;box-shadow:0 12px 40px rgba(27,36,48,0.06);">
        ...
        <tr><td style="padding:14px 26px;border-top:1px solid ${C.hairline};font-family:ui-sans-serif,system-ui,-apple-system,sans-serif;font-size:13px;line-height:1.55;color:${C.ink};">
          <span style="display:block;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${C.muted};margin-bottom:8px;">Public strategy call</span>
          <a href="${STRATEGY_CALL_BOOKING_URL}" style="color:${C.primary};font-weight:600;text-decoration:none;border-bottom:1px solid ${C.primaryBorder};">${esc(STRATEGY_CALL_BOOKING_URL)}</a>
        </td></tr>
...
export function buildLeadEnquiryEmailText(input: LeadEnquiryEmailInput): string {
  ...
  const lines = [
    ...
    "",
    "PUBLIC STRATEGY CALL (canonical)",
