...
function briefFileName(): string {
  const stamp = new Date().toISOString().slice(0, 10)
  return `pxlbrief-ai-strategic-brief-${stamp}.html`
}
...
function buildStrategicBriefHtml(brief: StrategicBrief): string {
  const sections = [
    ["Business understanding", `<p>${escapeHtml(brief.businessUnderstanding)}</p>`],
    ["Current bottlenecks", listHtml(brief.currentBottlenecks)],
    ["Likely growth blockers", listHtml(brief.likelyGrowthBlockers)],
    ["Brand positioning observations", listHtml(brief.brandPositioningObservations)],
    ["Customer acquisition gaps", listHtml(brief.customerAcquisitionGaps)],
    ["Funnel / system weaknesses", listHtml(brief.funnelSystemWeaknesses)],
    ["Recommended strategic direction", `<p>${escapeHtml(brief.recommendedStrategicDirection)}</p>`],
    ["AI / automation opportunities", listHtml(brief.automationOpportunities)],
    ["Recommended service stack", listHtml(brief.recommendedServiceStack)],
    ["Suggested execution roadmap", listHtml(brief.suggestedExecutionRoadmap)],
  ] as const
...
<head>
  ...
  <title>PxlBrief AI Strategic Brief</title>
  <style>
    body { margin: 0; background: #071012; color: #ecfdf5; font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }
    .page { max-width: 880px; margin: 0 auto; padding: 48px 28px 64px; }
    .shell { border: 1px solid rgba(94, 234, 212, .18); border-radius: 28px; padding: 34px; background: linear-gradient(135deg, rgba(20, 184, 166, .13), rgba(15, 23, 42, .74)); box-shadow: 0 32px 90px rgba(0,0,0,.42); }
    .eyebrow { color: #5eead4; font-size: 11px; letter-spacing: .22em; text-transform: uppercase; font-weight: 700; }
    h1 { margin: 10px 0 10px; font-size: 38px; line-height: 1; letter-spacing: -.04em; }
    .meta { color: rgba(236,253,245,.68); line-height: 1.7; max-width: 680px; }
    .pills { display: flex; flex-wrap: wrap; gap: 8px; margin: 24px 0; }
    .pill { border: 1px solid rgba(94,234,212,.18); border-radius: 999px; padding: 8px 12px; color: #ccfbf1; background: rgba(94,234,212,.07); font-size: 12px; }
    .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-top: 26px; }
    .card { border: 1px solid rgba(255,255,255,.09); border-radius: 18px; padding: 18px; background: rgba(255,255,255,.035); break-inside: avoid; }
    h2 { margin: 0 0 10px; color: #ccfbf1; font-size: 13px; letter-spacing: .12em; text-transform: uppercase; }
    p, li { color: rgba(236,253,245,.82); font-size: 14px; line-height: 1.7; }
    ul { margin: 0; padding-left: 18px; }
    .footer { margin-top: 28px; color: rgba(236,253,245,.5); font-size: 12px; }
    @media print { body { background: #071012; } .page { padding: 0; } .shell { box-shadow: none; } }
    @media (max-width: 720px) { .grid { grid-template-columns: 1fr; } h1 { font-size: 30px; } .shell { padding: 24px; } }
  </style>
</head>
<body>
  <main class="page">
    <section class="shell">
      <div class="eyebrow">PxlBrief AI Strategic Brief</div>
      <h1>AI-generated strategic operating diagnosis</h1>
      <p class="meta">A premium founder intelligence snapshot generated from the live PxlBrief AI consultation.</p>
      <div class="pills">
        <span class="pill">Priority: ${escapeHtml(brief.recommendedPriority)}</span>
        ${brief.estimatedImpactAreas.map((area) => `<span class="pill">${escapeHtml(area)}</span>`).join("")}
      </div>
      <div class="grid">
        ${sections.map(([title, body]) => `<article class="card"><h2>${escapeHtml(title)}</h2>${body}</article>`).join("")}
      </div>
      <div class="footer">Generated by PxlBrief AI. Use as directional strategy input before execution planning.</div>
    </section>
  </main>
</body>
</html>`
...
function downloadStrategicBrief(brief: StrategicBrief) {
  if (typeof window === "undefined") return
  const blob = new Blob([buildStrategicBriefHtml(brief)], {
    ...
    <div className="flex justify-start gap-3 md:gap-3.5">
      ...
      <article className="w-full max-w-[min(92%,34rem)] overflow-hidden rounded-[1.05rem] border border-primary/18 bg-gradient-to-br from-primary/[0.1] via-card/[0.48] to-black/[0.16] p-4 text-left shadow-[0_20px_56px_-34px_rgba(0,0,0,0.85),inset_0_1px_0_0_rgba(255,255,255,0.055)] backdrop-blur-xl animate-in fade-in-0 slide-in-from-bottom-2 duration-500">
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full border border-primary/16 bg-primary/[0.065] px-2.5 py-1 text-[0.62rem] font-medium uppercase tracking-[0.12em] text-primary/82">
            <span
...
function BriefSection({
  ...
  return (
    <details
      open={defaultOpen}
      className="group rounded-[0.95rem] border border-white/[0.075] bg-white/[0.035] p-3 transition-colors duration-300 open:border-primary/18 open:bg-primary/[0.04]"
    >
      <summary className="cursor-pointer list-none text-[0.68rem] font-semibold uppercase tracking-[0.15em] text-primary/84 marker:hidden">
        <span className="inline-flex w-full items-center justify-between gap-3">
          {title}
          <span className="text-muted-foreground/55 transition-transform duration-300 group-open:rotate-45">
            +
          </span>
        </span>
      </summary>
      <div className="mt-3 text-[0.75rem] leading-relaxed text-muted-foreground/88">
        {children}
      </div>
    </details>
  )
...
function StrategicBriefList({ items }: { items: string[] }) {
  return (
    <ul className="space-y-2">
      {items.map((item) => (
        <li key={item} className="flex gap-2">
          <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-primary/75" />
          <span>{item}</span>
        </li>
      ))}
    </ul>
...
function StrategicBriefDocument({
  brief,
  isGenerating,
}: {
  ...
  if (!brief.isReady) return null

  return (
    <article className="mt-6 overflow-hidden rounded-[1.15rem] border border-primary/18 bg-gradient-to-br from-primary/[0.095] via-card/[0.42] to-black/[0.18] p-4 shadow-[0_22px_60px_-38px_rgba(0,0,0,0.88),inset_0_1px_0_0_rgba(255,255,255,0.055)] animate-in fade-in-0 slide-in-from-bottom-2 duration-500">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          <p className="mb-1 text-[0.625rem] font-semibold uppercase tracking-[0.2em] text-primary/88">
            AI Strategic Brief
          </p>
          <h4 className="text-base font-semibold tracking-[-0.015em] text-foreground">
            Generated intelligence document
          </h4>
        </div>
        <button
          type="button"
          disabled={isGenerating}
          onClick={() => downloadStrategicBrief(brief)}
          className="inline-flex shrink-0 items-center gap-1.5 rounded-[0.75rem] border border-white/[0.09] bg-white/[0.045] px-3 py-2 text-[0.68rem] font-semibold uppercase tracking-[0.12em] text-foreground/86 transition hover:border-primary/28 hover:bg-primary/[0.07] disabled:pointer-events-none disabled:opacity-45"
        >
      ...
      {isGenerating ? (
        <div className="rounded-[1rem] border border-primary/16 bg-black/20 p-4">
          <div className="mb-3 flex items-center gap-2">
            <span
              className="h-2 w-2 rounded-full bg-primary"
              style={{ animation: "pxl-dot-pulse 1.25s ease-in-out infinite" }}
            />
            <p className="text-[0.72rem] font-semibold uppercase tracking-[0.14em] text-primary/85">
        ...
        <>
          <div className="mb-4 rounded-[0.95rem] border border-white/[0.075] bg-black/20 p-3">
            <p className="mb-1 text-[0.58rem] font-medium uppercase tracking-[0.14em] text-muted-foreground/66">
              Business understanding
            </p>
            <p className="text-[0.8125rem] leading-relaxed text-foreground/90">
              {brief.businessUnderstanding}
            </p>
          </div>

          <div className="mb-4 grid gap-2 sm:grid-cols-2">
            <DiagnosisField label="Recommended priority" value={brief.recommendedPriority} />
            <DiagnosisField
              label="Estimated impact areas"
              value={brief.estimatedImpactAreas.join(" / ")}
            />
          </div>

          <div className="space-y-2">
            <BriefSection title="Current bottlenecks" defaultOpen>
              <StrategicBriefList items={brief.currentBottlenecks} />
            </BriefSection>
            <BriefSection title="Likely growth blockers">
              <StrategicBriefList items={brief.likelyGrowthBlockers} />
            </BriefSection>
            <BriefSection title="Brand positioning observations">
              <StrategicBriefList items={brief.brandPositioningObservations} />
            </BriefSection>
            <BriefSection title="Customer acquisition gaps">
              <StrategicBriefList items={brief.customerAcquisitionGaps} />
            </BriefSection>
            <BriefSection title="Funnel / system weaknesses">
              <StrategicBriefList items={brief.funnelSystemWeaknesses} />
            </BriefSection>
            <BriefSection title="Recommended strategic direction" defaultOpen>
              {brief.recommendedStrategicDirection}
            </BriefSection>
            <BriefSection title="AI / automation opportunities">
              <StrategicBriefList items={brief.automationOpportunities} />
            </BriefSection>
            <BriefSection title="Recommended service stack">
              <StrategicBriefList items={brief.recommendedServiceStack} />
            </BriefSection>
            <BriefSection title="Suggested execution roadmap" defaultOpen>
              <StrategicBriefList items={brief.suggestedExecutionRoadmap} />
            </BriefSection>
          </div>
        </>
      )}
      ...
      <div className="mb-4 rounded-[1rem] border border-primary/14 bg-primary/[0.055] p-3">
        ...
        <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.06]">
          <div
            ...
            style={{
              width: intelligence.isReady ? "76%" : "42%",
      ...
      {!intelligence.isReady ? (
        ...
        <>
          <div className="mb-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
            <DiagnosisField
              label="Estimated growth blockers"
              value={intelligence.estimatedGrowthBlockers[0] ?? "Monitoring business constraints"}
            />
            <DiagnosisField
              label="Estimated missed revenue areas"
              value={intelligence.estimatedMissedRevenueAreas[0] ?? "Monitoring commercial leakage"}
            />
            <DiagnosisField
              label="Estimated operational inefficiencies"
              value={intelligence.estimatedOperationalInefficiencies[0] ?? "Monitoring workflow gaps"}
            />
            <DiagnosisField
              label="Suggested strategic priorities"
              value={intelligence.suggestedStrategicPriorities[0] ?? "Mapping advisory priorities"}
            />
          </div>
    ...
    <aside className="relative overflow-hidden rounded-[1.25rem] border border-primary/18 bg-gradient-to-b from-card/[0.66] via-card/[0.42] to-black/[0.24] p-4 text-left shadow-[0_28px_72px_-36px_rgba(0,0,0,0.9),0_0_54px_-34px_oklch(0.75_0.12_180/0.9),inset_0_1px_0_0_rgba(255,255,255,0.055)] backdrop-blur-2xl animate-in fade-in-0 slide-in-from-bottom-2 duration-500 lg:sticky lg:top-20 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto lg:p-4 xl:p-5">
      ...
      <div className="relative">
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full border border-primary/18 bg-primary/[0.075] px-2.5 py-1 text-[0.62rem] font-medium uppercase tracking-[0.12em] text-primary/86 shadow-[0_0_34px_-22px_oklch(0.75_0.12_180/0.95)]">
            <span
              className="h-1.5 w-1.5 rounded-full bg-primary"
              style={{ animation: "pxl-dot-pulse 1.35s ease-in-out infinite" }}
            />
        ...
        <div className="mb-3 grid grid-cols-2 gap-2">
          <DiagnosisField label="AI Diagnosis" value={diagnosis.mainBottleneck} />
          <DiagnosisField label="Growth Potential" value={diagnosis.recommendedGrowthDirection} />
          <DiagnosisField label="Strategic Priority" value={intelligence.suggestedStrategicPriorities[0] ?? diagnosis.serviceFit} />
          <DiagnosisField label="Analysis Status" value="Strategic analysis active" />
        </div>

        <div className="mb-3 grid grid-cols-2 gap-2">
          <DiagnosisField label="Industry" value={diagnosis.industry} />
          <DiagnosisField label="Business stage" value={diagnosis.businessStage} />
          <DiagnosisField label="Revenue Leak Signals" value={diagnosis.potentialRevenueLeaks[0] ?? "Monitoring"} />
        ...
        <div className="mb-4 rounded-[1rem] border border-primary/14 bg-primary/[0.055] p-3">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <span className="flex items-center gap-1.5 text-[0.62rem] uppercase tracking-[0.12em] text-primary/82">
              <span
                className="h-1.5 w-1.5 rounded-full bg-primary"
                style={{ animation: "pxl-dot-pulse 1.35s ease-in-out infinite" }}
              />
...
export function HeroSection() {
  ...
  const [strategicBriefGenerating, setStrategicBriefGenerating] =
    useState(false)
  const leadPrepFingerprintRef = useRef<string>("")
  const briefFingerprintRef = useRef<string>("")
  ...
  useEffect(() => {
    if (!strategicBrief.isReady) {
      setStrategicBriefGenerating(false)
      return
    }

    const fingerprint = [
      messages.length,
      aiDiagnosis.mainBottleneck,
      aiDiagnosis.recommendedGrowthDirection,
      strategicBrief.recommendedPriority,
    ].join("|")

    if (fingerprint === briefFingerprintRef.current) return
    briefFingerprintRef.current = fingerprint
    setStrategicBriefGenerating(true)
                ...
                <div className="flex flex-wrap items-center justify-end gap-2 sm:ml-auto">
                  {(leadPrepBusy || leadIntel) &&
                    conversationState.visitorType !== "unknown" && (
                      <span
                        className="inline-flex items-center gap-1.5 rounded-full border border-primary/14 bg-primary/[0.055] px-2.5 py-1 text-[0.625rem] font-medium uppercase tracking-[0.12em] text-primary/78 md:text-[0.6875rem]"
                        aria-live="polite"
                      >
                        <span
                          className="h-1.5 w-1.5 rounded-full bg-primary"
                          style={{
                            animation: "pxl-dot-pulse 1.35s ease-in-out infinite",
                          }}
                        />
                        Strategic analysis active
                      </span>
